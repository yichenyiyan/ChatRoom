...
yichen_dialog::yichen_dialog(QWidget *parent, Qt::WindowFlags f)
    : QDialog(parent,f), status(false), port(8888), IP("127.0.0.1") {
    this->resize(400, 400);
...
void
yichen_dialog::slotDisconnected() {
    sendBtn->setEnabled(false);
    enterBtn->setText(tr("Enter Chat Room"));
}

void
yichen_dialog::slotClear() {
    contentListWidget->clear();
}

void
yichen_dialog::dataReceived() {
