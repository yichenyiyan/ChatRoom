...
void Menu::setupUI() {
    ...
    QPushButton *groupAddButton = new QPushButton("加入群聊", functionsPage);
    QPushButton *groupCreateButton = new QPushButton("创建群聊", functionsPage);
    ...
    functionsLayout->addWidget(groupAddButton);
    functionsLayout->addWidget(groupCreateButton);
...
void Menu::setupConnections() {
    ...
    connect(static_cast<QPushButton*>(functionsLayout->itemAt(3)->widget()), &QPushButton::clicked, this, &Menu::onAddGroupButtonClicked);
    connect(static_cast<QPushButton*>(functionsLayout->itemAt(4)->widget()), &QPushButton::clicked, this, &Menu::onCreateGroupButtonClicked);
...
void Menu::onGroupChatButtonClicked() {
    // 群聊聊天
    GroupListWindow* _groups = new GroupListWindow(user, messagesPage, nullptr);
    _groups->show();
}

void Menu::onAddGroupButtonClicked() {
    // 添加群聊
    JoinGroupChatWindow* jwin = new JoinGroupChatWindow(user, nullptr);
    jwin->show();
}


void Menu::onCreateGroupButtonClicked() {
    // 创建群聊
    CreateGroupChatWindow* cwin = new CreateGroupChatWindow(user, nullptr);
    cwin->show();
}
