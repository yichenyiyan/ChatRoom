...
void TcpClientThread::handleMessage() {
    ...
    } else if (messageType == '3' && parts.size() >= 2) {
        QString groupName = parts[0];
        QString message = parts.mid(1).join(" ");
        emit newMessageReceived3(groupName, message);
    } else if (messageType == 'd' && parts.size() >= 2) {
        QString groupName = parts[0];
        QString message = parts.mid(1).join(" ");
        emit newMessageReceived4(groupName, message);
    }
}
