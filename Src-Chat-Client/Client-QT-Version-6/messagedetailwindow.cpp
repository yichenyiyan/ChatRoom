...
MessageDetailWindow::MessageDetailWindow(const QString& _sendUsername, const QString& _friendName, bool isGroup, QWidget *parent)
    : QWidget(parent), isGroupWin(isGroup), sendUsername(_sendUsername), friendUserName(_friendName) {
...
void MessageDetailWindow::setupUI() {
    ...
    if (!isGroupWin) {
        ...
        connect(sendButton, &QPushButton::clicked, this, [&]() {
            ...
            sendSocket->sendCommand("3 " + sendUsername + " " + friendUserName + " "+ message);
...
void MessageDetailWindow::addMessage(const QString &username, const QString &message, bool isOwnMessage) {
    if (username != friendUserName || isOwnMessage && isGroupWin) return;
