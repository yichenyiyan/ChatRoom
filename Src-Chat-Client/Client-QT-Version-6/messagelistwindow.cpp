...
MessageListWindow::MessageListWindow(const QString& _user, QWidget *parent)
    ...
    connect(tcpClientThread, &TcpClientThread::newMessageReceived3, this, &MessageListWindow::addGMessage);
    connect(tcpClientThread, &TcpClientThread::newMessageReceived4, this, &MessageListWindow::addGroupWin);
...
void MessageListWindow::addGMessage(const QString &username, const QString &message) {
    QPushButton *messageButton;
    QDate currentDate = QDate::currentDate();
    QTime currentTime = QTime::currentTime();
    QString displayDate = currentDate.toString("yyyy-MM-dd");
    ...
    if (userButtons.contains(username)) {
        ...
        messageButton->setStyleSheet("background-color: rgba(0, 0, 0, 0); border: 1px solid black; color: black; text-align: left; border-radius: 10px;");
        connect(messageButton, &QPushButton::clicked, this, [this, username]() {
            openGMessageDetailWindow(username);
        });
...
void MessageListWindow::toAddGMessage(const QString &username, const QString &message) {
    QPushButton *messageButton;
    QDate currentDate = QDate::currentDate();
    QTime currentTime = QTime::currentTime();
    QString displayDate = currentDate.toString("yyyy-MM-dd");
...
void MessageListWindow::openMessageDetailWindow(const QString &username) {
    ...
    if (userWindows.contains(username)) {
        ...
        detailWindow = new MessageDetailWindow(user, username);
        userWindows[username] = detailWindow;
    }
    detailWindow->show();
}

void MessageListWindow::openGMessageDetailWindow(const QString &username) {
    MessageDetailWindow *detailWindow;
    if (userWindows.contains(username)) {
        detailWindow = userWindows[username];
    } else {
        detailWindow = new MessageDetailWindow(user, username, true);
        userWindows[username] = detailWindow;
    }
    detailWindow->show();
}
...
void MessageListWindow::toOpenGMessageDetailWindow(const QString &username) {
    MessageDetailWindow *detailWindow;
    if (userWindows.contains(username)) {
        detailWindow = userWindows[username];
    } else {
...
void MessageListWindow::addGroupWin(const QString& req_user, const QString& groupname) {
    GroupRequestWindow* addgroup = new GroupRequestWindow(user, req_user, groupname, nullptr);
    addgroup->show();
}
